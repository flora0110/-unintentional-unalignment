...
class SequenceResp0LogitsAndProbsTrackerCallback(Callback):
    ...
    def __init__(self, tokenizer, logger: logging.Logger, num_inputs_to_log_logit_change_for: int = 1,
                 top_tokens_to_log: int = 10, top_items_to_log: int = 20,
                 epoch_log_interval: int = 1, log_after_first_epoch: bool = True,
                 track_finegrained_token_metrics: bool = False,
                 policy_model: Optional[nn.Module] = None,
                 reference_model: Optional[nn.Module] = None,
                 id2name_path: Optional[str] = None,
    ...
    def on_train_batch_end(self, trainer: Trainer, batch_num: int, batch_output, metric_values):
        ...
        train_loss_raw = batch_output["train loss"]
        unembedding_weights = batch_output["unembedding weights"]
        hidden_representations = batch_output["resp0_hidden"]            # [B, H]

        train_loss = float(train_loss_raw if isinstance(train_loss_raw, (int, float)) else train_loss_raw.item())
